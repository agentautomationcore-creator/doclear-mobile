...
export default function DocumentDetailScreen() {
  ...
  const handleShare = useCallback(async () => {
    ...
    const text = `${doc.title}\n\n${t('document.health_score')}: ${doc.healthScore ?? '?'}/100\n\n${t('document.key_facts')}:\n${facts}\n\nDocLear - doclear.app`;
    ...
    try {
      ...
      if (__DEV__) console.error('[DocDetail] share error:', e);
    }
  }, [doc]);
  ...
  const handleExportPDF = useCallback(async () => {
    ...
    const html = `
      <html><head><meta charset="utf-8"><style>
        ...
        <div class="disclaimer">${t('document.pdf_disclaimer')}</div>
        <p style="margin-top:32px;color:#64748b;font-size:12px;">${t('document.pdf_generated_by')}</p>
      </body></html>
    `;
    try {
      await Print.printAsync({ html });
    } catch (e) {
      if (__DEV__) console.error('[DocDetail] PDF export error:', e);
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: COLORS.background }} edges={['top']}>
      ...
      >
        ...
        <Card style={{ marginBottom: 16 }}>
          ...
          >
            <Text style={{ fontSize: 11, fontWeight: '600', color: '#1e40af' }}>
              {doc.docTypeLabel ?? doc.docType ?? t('document.default_type')}
            </Text>
          </View>
          {doc.summary ? (
        ...
        {/* Analysis incomplete fallback */}
        {!doc.whatIsThis && !doc.whatItSays ? (
          <Card style={{ marginBottom: 16, backgroundColor: '#FEF3C7' }}>
            <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8, marginBottom: 8 }}>
              <Ionicons name="warning-outline" size={20} color="#92400E" />
              <Text style={{ fontSize: FONT_SIZE.body, fontWeight: '700', color: '#92400E' }}>
                {t('document.ai_analysis')}
              </Text>
            </View>
            <Text style={{ fontSize: FONT_SIZE.caption, color: '#92400E', lineHeight: 20 }}>
              {t('document.analysis_incomplete')}
            </Text>
          </Card>
        ) : null}
              ...
              onPress={async () => {
                ...
                try {
                  const { data: profile } = await supabase
                    ...
                    .single();
                  city = profile?.city || '';
                } catch (e) { if (__DEV__) console.error('[DocDetail] profile fetch error:', e); }
      ...
      <SafeAreaView style={{ flex: 1, backgroundColor: '#FFFFFF' }}>
        ...
        >
          ...
          >
            ...
            />
            <Text style={{ fontSize: 12, color: '#64748B', textAlign: 'center', paddingHorizontal: 16, paddingBottom: 4 }}>
